...
//   no_of_job_applications?: number;
//   application_sale_value?: number;
//   resume_sale_value?: number;
//   portfolio_sale_value?: number;
//   linkedin_sale_value?: number;
//   github_sale_value?: number;
//   courses_sale_value?: number;
...
//         setObSalaryRange(onboarding.salary_range || "");
//         setObWorkAuth(onboarding.work_auth_details || "");
//         setObNeedsSponsorship(onboarding.needs_sponsorship ?? onboarding.sponsorship ?? null);
//         setObFullAddress(onboarding.full_address || "");
//         setObLinkedInUrl(onboarding.linkedin_url || "");
//         setObDob(onboarding.date_of_birth || "");
//         setObWhatsappNumber(onboarding.whatsapp_number || "");
...
//         const allowedVisaTypes = ["F1", "H1B", "Green Card", "Citizen", "H4EAD", "Other"];
//         const rawVisa = onboarding.visatypes || onboarding.visa_type || "";
//         setObVisaType(allowedVisaTypes.includes(rawVisa) ? rawVisa : rawVisa ? "Other" : "");
...
//           setObPersonalEmail(sales.email);
//         }
//         if (sales.no_of_job_applications) {
//           setObNoOfApplications(sales.no_of_job_applications.toString());
//         }
//       }
...
//       if (!rolesRes.ok) {
//         throw new Error(`Roles API failed with status: ${rolesRes.status}`);
//       }
...
//     if (!latestResumePath || latestResumePath.trim() === "") {
//       throw new Error("Resume is required before onboarding. Please upload a resume first.");
//     }
...
//     endDate: string | null
//   ) => {
//     // ==================== ROLE VALIDATION - STRICT MATCHING ====================
//     let isNewDomain = false;
//     let jobRoles: string[] = [];

//     // Get the primary role from form or existing data
//     const allRoles = csvToArray(obJobRolesText || fullOnboardingData?.job_role_preferences || fullOnboardingData?.role || "");
//     const primaryRoleInput = allRoles.length > 0 ? allRoles[0] : "";

//     console.log("📋 Primary Role Input:", primaryRoleInput);

//     if (primaryRoleInput) {
//       // Validate role with STRICT matching
//       const { exists } = await validateRoleStrict(primaryRoleInput);

//       isNewDomain = !exists;
//       jobRoles = [primaryRoleInput]; // Keep EXACT input, no modification

//       console.log("🎯 Role Validation Result:", {
//         role: primaryRoleInput,
//         existsInAPI: exists,
//         isNewDomain,
//         action: exists ? "Existing Domain" : "New Domain"
//       });
//     }

//     // ==================== BUILD PAYLOAD ====================
//     const payload: any = {
//       // 1. Spread ALL data from DB
//       ...fullOnboardingData,

//       // 2. Required fields with form state
//       full_name: obFullName || fullOnboardingData?.full_name || "",
//       email: obCompanyEmail?.trim() || fullOnboardingData?.company_email || "",
//       personal_email: obPersonalEmail || fullOnboardingData?.personal_email || "",
//       experience: obExperience || fullOnboardingData?.experience || "0",
...
//       // 3. Role fields - using validated data
//       job_role_preferences: jobRoles,
//       target_role: jobRoles.join(", ") || "Unknown",
//       is_new_domain: isNewDomain,

//       // 4. Location and preferences
//       location_preferences: csvToArray(obLocationsText),
//       visa_type: obVisaType || fullOnboardingData?.visatypes || fullOnboardingData?.visa_type || "Other",
//       sponsorship: obNeedsSponsorship ?? fullOnboardingData?.sponsorship ?? fullOnboardingData?.needs_sponsorship ?? false,
...
//       // 7. Sales and subscription data
//       add_ons_info: addOnsInfo,
//       no_of_applications: scData?.no_of_job_applications?.toString() || obNoOfApplications || "20",
//       badge_value: scData?.badge_value ?? fullOnboardingData?.badge_value ?? 0,
...
//       { val: payload.resume_s3_path, msg: "Resume is required before onboarding. Please upload a resume first." },
//       { val: payload.start_date, msg: "Onboarded Date (Start Date) is required before onboarding." },
//       { val: payload.target_role && payload.target_role !== "Unknown", msg: "Target Role / Job Role Preference is required before onboarding." },
//       { val: payload.no_of_applications, msg: "Number of Applications is required before onboarding." },
//       { val: payload.add_ons_info && payload.add_ons_info.length > 0, msg: "Services Opted (Add-Ons) is required. Please check the Sales Closure record." },
//       { val: payload.work_auth_details, msg: "Work Authorization Details is required before onboarding." },
//       { val: payload.work_preferences, msg: "Work Preference (Remote/Hybrid/On-site) is required before onboarding." },
//     ];
...
//       // 7. Sync with External API
//       console.log("📤 Submitting Onboarding Payload:", apiPayload);
//       await sendDirectOnboard(apiPayload);
//       console.log("✅ API Sync Successful");

//       // 8. Update client_onborading_details
//       const supabaseOnboardingPayload = {
//         ...(fullOnboardingData || {}),
//         full_name: obFullName || null,
//         company_email: obCompanyEmail?.trim() || null,
...
//           .from("client_onborading_details")
//           .update(supabaseOnboardingPayload)
//           .eq("id", onboardingData.id);
//         if (error) throw error;
//       } else {
//         const { error } = await supabase
//           .from("client_onborading_details")
//           .insert(supabaseOnboardingPayload);
//         if (error) throw error;
//       }
...
//                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
//                   <div className="space-y-2">
//                     <Label htmlFor="fullName" className="after:content-['*'] after:ml-0.5 after:text-red-500">
//                       Full Name
//                     </Label>
//                     <Input
//                       id="fullName"
...
//                   </div>
//                   <div className="space-y-2">
//                     <Label htmlFor="companyEmail" className="after:content-['*'] after:ml-0.5 after:text-red-500">
//                       Company Email
//                     </Label>
//                     <Input
//                       id="companyEmail"
...
//                   </div>
//                   <div className="space-y-2">
//                     <Label htmlFor="onboardedDate" className="after:content-['*'] after:ml-0.5 after:text-red-500">
//                       Onboarded Date
//                     </Label>
//                     <Input
//                       id="onboardedDate"
...
//                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
//                   <div className="space-y-2">
//                     <Label htmlFor="jobRoles" className="after:content-['*'] after:ml-0.5 after:text-red-500">
//                       Job Role Preferences (comma separated)
//                     </Label>
//                     <Textarea
//                       id="jobRoles"
//                       rows={3}
...
//                       First role will be used for domain validation
//                     </p>
//                   </div>
//                   <div className="space-y-2">
//                     <Label htmlFor="locations">Location Preferences (comma separated)</Label>
...
//                   </div>
//                   <div className="space-y-2">
//                     <Label htmlFor="workPreference" className="after:content-['*'] after:ml-0.5 after:text-red-500">
//                       Work Preference
//                     </Label>
//                     <Select
//                       value={obWorkPreference}
...
//                   </div>
//                   <div className="space-y-2">
//                     <Label htmlFor="workAuth" className="after:content-['*'] after:ml-0.5 after:text-red-500">
//                       Work Auth Details
//                     </Label>
//                     <Textarea
//                       id="workAuth"
...
//                   </div>
//                   <div className="space-y-2">
//                     <Label htmlFor="visaType" className="after:content-['*'] after:ml-0.5 after:text-red-500">
//                       Visa Type
//                     </Label>
//                     <Select
//                       value={obVisaType || "__unset__"}
...
//                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
//                   <div className="space-y-2">
//                     <Label htmlFor="gender" className="after:content-['*'] after:ml-0.5 after:text-red-500">
//                       Gender
//                     </Label>
//                     <Select
//                       value={obGender || "__unset__"}
...
//                   </div>
//                   <div className="space-y-2">
//                     <Label htmlFor="stateOfResidence" className="after:content-['*'] after:ml-0.5 after:text-red-500">
//                       State of Residence
//                     </Label>
//                     <Input
//                       id="stateOfResidence"
...
//                   </div>
//                   <div className="space-y-2">
//                     <Label htmlFor="zipOrCountry" className="after:content-['*'] after:ml-0.5 after:text-red-500">
//                       Zip or Country
//                     </Label>
//                     <Input
//                       id="zipOrCountry"
...
export default function OnboardingForm() {
   ...
   * - Production-ready with timeout, detailed logging, and toast feedback
   */
  const validateRoleStrict = async (role: string): Promise<{ exists: boolean; validRoles: string[]; debug: any }> => {
    if (!role || role.trim() === "") {
      return { exists: false, validRoles: [], debug: { error: "Empty role" } };
    ...
    try {
      ...
      const rolesData = await rolesRes.json();
      console.log("📦 Raw API Response type:", Array.isArray(rolesData) ? "array" : typeof rolesData);

      // FIXED & ROBUST PARSING (confirmed: top-level is always an array of objects)
      let rolesList: any[] = [];
      if (Array.isArray(rolesData)) {
        rolesList = rolesData;
      } else if (rolesData?.data && Array.isArray(rolesData.data)) {
      ...
      console.log("📋 Extracted roles list length:", rolesList.length);

      // Extract ONLY canonical names (strict - no alternateRoles or keywords)
      const validRoles: string[] = rolesList
        .map((item: any) => {
      ...
      console.log("✅ Valid roles extracted count:", validRoles.length);
      console.log("Sample valid roles (first 5):", validRoles.slice(0, 5));

      // STRICT EXACT MATCH (case-sensitive, trimmed input only)
      const exists = validRoles.includes(trimmedRole);

      // Find matching object for debug
      const matchingRole = rolesList.find((item: any) =>
        item && typeof item === "object" && item.name === trimmedRole
      );

      const debug = {
        inputRole: trimmedRole,
        exists,
        matchingRoleId: matchingRole?.id || null,
        matchingRoleName: matchingRole?.name || null,
        validRolesCount: validRoles.length,
        validRolesSample: validRoles.slice(0, 10),
        rawResponseType: Array.isArray(rolesData) ? "array" : typeof rolesData,
        totalRolesFetched: rolesList.length,
      };

      console.log("🔍 Role Validation Debug:", debug);

      if (exists) {
        toast.success(`✅ Role "${trimmedRole}" found in API! (ID: ${matchingRole?.id || "N/A"})`);
      } else {
        toast.error(`❌ Role "${trimmedRole}" NOT found in API. Check console for available roles.`);
      }
  ...
  const validateAndBuildPayload = async (
    ...
    if (primaryRoleInput) {
      // Validate role with STRICT matching (fixed parser)
      const { exists } = await validateRoleStrict(primaryRoleInput);
      isNewDomain = !exists;
      jobRoles = [primaryRoleInput]; // Keep EXACT input, no modification
      console.log("🎯 Role Validation Result:", {
